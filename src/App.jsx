...
function LoginScreen({ onBack }) {
  ...
  return (
    <div style={{ maxWidth: '390px', margin: '0 auto', fontFamily: 'sans-serif', padding: '80px 24px 0' }}>
      ...
      <button onClick={handleLogin} style={{ display: 'flex', alignItems: 'center', gap: '12px', margin: '0 auto', padding: '14px 24px', borderRadius: '12px', border: '1px solid #ddd', background: '#fff', fontSize: '15px', fontWeight: 500, cursor: 'pointer', width: '100%', justifyContent: 'center' }}>
...
function App() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, u => {
      setUser(u)
      if (u && screen === 'login') setScreen('add')
    })
    fetchAds()
    return () => unsub()
  }, [])
